...
def generate():
    text["text"] = Figlet(font=font_value.get()).renderText(text=text_value.get())
def copy():
    pyperclip.copy(text['text'])
    messagebox.showinfo("INFO", "Text copied to clipboard")

name = Label(text="Font generator").pack()
...
ok_btn = Button(text="generate",command=generate).pack()
text = Label(text="")
text.pack() 
save_btn = Button(text="copy",command=copy).pack()
